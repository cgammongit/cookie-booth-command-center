...
export async function POST(
  ...
  try {
    await (await clerkClient()).invitations.revokeInvitation(
  ...
  await env.DB.batch([
    ...
    `).bind(now, now, invitationId, organizationId),
    env.DB.prepare(`
      INSERT INTO access_audit_log (
        organization_id, actor_user_id, target_membership_id, action,
        before_json, after_json, created_at
      ) VALUES (?, ?, ?, 'invitation_cancelled', ?, ?, ?)
    `).bind(
      organizationId,
      authorization.access.userId,
      invitation.membershipId,
      JSON.stringify({ status: "pending", email: invitation.email }),
      JSON.stringify({ status: "cancelled", email: invitation.email }),
      now,
    ),
  ]);
